...
def download_packages(packages_data, download_base_path, max_workers=20):
    ...
    for package_name, versions in packages_data.items():
        package_version_count = len(versions)  # 获取该包的版本数量
        for version, file_info in versions.items():
            download_tasks.append((package_name, version, file_info, download_base_path, package_version_count))
...
def print_summary(packages_data):
    ...
    # 统计单版本和多版本包
    single_version_packages = 0
    multi_version_packages = 0
    
    for package_name, versions in packages_data.items():
        if len(versions) == 1:
            single_version_packages += 1
        else:
            multi_version_packages += 1
    
    print(f"总包数量: {total_packages}")
    print(f"总版本数量: {total_versions}")
    print(f"📁 单版本包: {single_version_packages} (将保存到 single-version/)")
    print(f"📁 多版本包: {multi_version_packages} (将保存到 multi-version/)")
    ...
    for i, (package_name, versions) in enumerate(list(packages_data.items())[:10], 1):
        ...
        version_type = "single-version" if version_count == 1 else "multi-version"
        print(f"  {i:2d}. {package_name} ({version_count} 个版本) → {version_type}/")
